"""
Error Types: Baseline vs FedAvg vs FedErrorAvg (TT / Test)
...
- One combined CSV for all datasets (aggregated)
Uses FED_AVG_CHECK_TT (baseline + FedAvg) and FED_ERRORAVG_CHECK_TT (FedErrorAvg).
"""
...
def main():
    ...
    for dataset_name, (baseline_path, fedavg_path, federror_path) in DATASETS.items():
        ...
        out_path = os.path.join(OUTPUT_DIR, f"error_types_{safe_name}_tt.csv")
        df_ds.to_csv(out_path, index=False)
        print(f"  - error_types_{safe_name}_tt.csv ({len(rows)} rows)")

    df_all = pd.DataFrame(all_rows)
    all_path = os.path.join(OUTPUT_DIR, "error_types_all_tt.csv")
    df_all.to_csv(all_path, index=False)
    print(f"  - error_types_all_tt.csv ({len(all_rows)} rows)")
    ...
    agg_path = os.path.join(OUTPUT_DIR, "error_types_combined_tt.csv")
    agg.to_csv(agg_path, index=False)
    print(f"  - error_types_combined_tt.csv (aggregated across datasets)")
