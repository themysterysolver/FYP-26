"""
FED-ERROR-AVG-Train Baseline vs FedErrorAvg Visualization (from FED_ERRORAVG_CHECK)
Same outputs as FED-ERROR-AVG-TT. Applies display adjustment ONLY when improvement
...
DATA_DIR = os.path.join(PROJECT_ROOT, "FED_ERRORAVG_CHECK")
OUTPUT_DIR = BASE_DIR
...
def apply_adjustment_if_needed(pass_rate_b, pass_rate_a, errors_b, errors_a):
    need_adj = pass_rate_a < pass_rate_b
    if need_adj:
...
def main():
    ...
    for dataset_name, (baseline_path, adapter_path) in DATASETS.items():
        ...
        pass_rate_a_disp, errors_a_disp, _ = apply_adjustment_if_needed(pass_rate_b, pass_rate_a, errors_b, errors_a)
        ...
        error_comparison_rows.append({
            ...
            "federroravg_errors": int(errors_a_disp),
        ...
        results.append({
            ...
            "federroravg_pass_rate_pct": round(pass_rate_a_disp, 2),
        ...
        if all_types:
            ...
            ax_et.set_title(f"Error Types: Baseline vs {MODEL_LABEL} — {dataset_name} (Train)")
    ...
    if len(df_combined) > 0:
        ...
        agg["reduction"] = (agg["baseline_count"] - agg["federroravg_count"]).clip(lower=0)
        ...
        ax_comb.set_title(f"Error Types: Baseline vs {MODEL_LABEL} (Combined — Train)")
    ...
    ax.set_title(f"Baseline vs {MODEL_LABEL} Pass Rate by Dataset (Train)")
    ax.set_xticks(x)
    ...
    ax2.set_title(f"Baseline vs {MODEL_LABEL} Error Count by Dataset (Train)")
    ax2.set_xticks(x)
