"""
FED-AVG-TT Baseline vs FedAvg Visualization
Generates: (1) Double bar graphs comparing baseline vs FedAvg pass rate
           (2) Error count comparison before/after FedAvg
...
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(BASE_DIR)), "FED_AVG_CHECK_TT")
OUTPUT_DIR = BASE_DIR
...
def main():
    ...
    for dataset_name, (baseline_path, adapter_path) in DATASETS.items():
        ...
        # Load full data for error type extraction
        ...
        results.append({
            ...
            "fedavg_pass_rate_pct": round(pass_rate_a, 2),
        ...
        # Error type breakdown: extract and count per type
        ...
        # Union of all error types for consistent x-axis
        ...
        if all_types:
            ...
            ax_et.set_title(f"Error Types: Baseline vs FedAvg — {dataset_name}")
    ...
    if len(df_combined) > 0:
        ...
        # Combined error types PNG: aggregate across all datasets per error type
        agg = df_combined.groupby("error_type").agg(
            baseline_count=("baseline_count", "sum"),
            fedavg_count=("fedavg_count", "sum"),
        ).reset_index()
        agg["reduction"] = agg["baseline_count"] - agg["fedavg_count"]
        ...
        ax_comb.set_title("Error Types: Baseline vs FedAvg (Combined — HumanEval + MBPP + DS1000)")
    ...
    # Plot 1: Double bar graph - Baseline vs FedAvg pass rate %
    ...
    ax.set_title("Baseline vs FedAvg Pass Rate by Dataset")
    ax.set_xticks(x)
    ...
    # Plot 2: Error count comparison - Baseline vs FedAvg errors
    ...
    ax2.set_title("Baseline vs FedAvg Error Count by Dataset")
    ax2.set_xticks(x)
