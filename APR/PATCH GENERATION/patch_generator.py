...
def load_and_merge_data(fault_info_path: str, master_table_path: str) -> pd.DataFrame:
    ...
    fault_df = pd.read_csv(fault_info_path, encoding="latin1")

    
    print("Loading hallucination_master_table.csv...")
    #master_df = pd.read_csv(master_table_path)
    master_df = pd.read_csv(master_table_path, encoding="latin1")
...
def main():
    ...
    # File paths
    fault_info_path = PROJECT_ROOT / "Hallucination detection" / "Fault Information" / "fault_information.csv"
    master_table_path = PROJECT_ROOT / "APR" / "ANALYSIS" / "hallucination_master_table.csv"
    output_path = PROJECT_ROOT / "APR" / "PATCH GENERATION" / "patched_code.csv"
